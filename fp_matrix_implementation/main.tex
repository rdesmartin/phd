...
\newcommand{\checkinn}{\textbf{CheckINN }}
...
This implementation allows us to define other useful functions concisely. For instance, the dot-product of two matrices %, or the $L_0$ distance between two matrices are
is defined as:
...
Note the use of the \lstinline{arg} type, which treats a matrix as a function evaluating ``queries'' (e.g., ``how many rows does this matrix have?'' or ``what is the value at index $(i,j)$?''). This formalisation technique is used as Imandra's logic does not allow function values inside of algebraic data types. % such as records.
...
\begin{table}\label{tab:acas_xu}
	...
	\begin{tabular}{{ P{1.5cm}  P{2cm}  P{2cm} P{1.5cm}  P{2cm} P{1.5cm} }}
		\toprule
		\multicolumn{2}{P{3.6cm}}{} & \multicolumn{2}{P{3.6cm}}{\textbf{CheckINN: Pruned Networks}} & \multicolumn{2}{P{3.6cm}}{\textbf{Reluplex: Full ACAS Xu Networks}} \\
...
%The loss of scalability was observed experimentally.
...
%introduce CheckINN in more detail
...
%For example, in all of these systems, reasoning about matrices represented as lists will require the development of a library of lemmas (with many proven by induction) giving algebraic laws for matrix element access and manipulation. As we have observed, functions defining core matrix operations (e.g., convolution) are often necessarily more complex over lists than their counterparts over (matrices represented as) functions; indeed, in the latter representation, many core functions can be defined in a non-recursive way, which can have major payoff for theorem proving efficiency.
...
\begin{table*}
	...
	\begin{tabular}{{ P{1.5cm}  P{2cm}  P{2cm}  P{1.5cm}  P{2cm} P{1.5cm}  P{2cm} P{1.5cm} }}
		\toprule
		\multicolumn{2}{P{3.6cm}}{} & \multicolumn{2}{{P{3.6cm}}}{\textbf{Imandra: Pruned and Quantized Networks}} & \multicolumn{2}{P{3.6cm}}{\textbf{Imandra: Pruned Networks}} & \multicolumn{2}{P{3.6cm}}{\textbf{Reluplex: Full ACAS Xu Networks}} \\
