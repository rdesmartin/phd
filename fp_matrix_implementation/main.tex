...
\rnote{It is correct; I added a sentence clarifying that we do not use the exact same matrix implementation}
...
\noindent The sparsity effect is achieved by \emph{pruning} the network, i.e. removing weights that have the smallest impact on the network's performance. We select weight's magnitude to select those to be pruned. This method, though rather rudimentary, is considered a reasonable pruning technique \cite{lecun_optimal_1990}. For the ACAS Xu problem, we are limited to a single pruning step, and cannot evaluate the pruned network's accuracy as no training data is accessible. We leave the evaluation of pruning techniques in relation to verfication to further studies. 

We do this mainly in order to reduce the amount of computation Imandra needs to perform, and to make the verification problem amenable by Imandra. However, unlike the  \lstinline{[@@blast]} experiment, this code generically works for any types of values, integer or real. 
\rnote{some adaptation is needed for real numbers; worth mentioning?}
...
This is a considerable improvement compared to the previous section. It is especially impressive that it comes ``for free'' with simply changing the underlying matrix representations.
...
A third implementation of matrices is by using records, a data structure that groups together values of different types. Standard OCaml records are available in Imandra, though they does not support functions as fields. In addition to records, we use Imandra's implementation of \emph{maps}. 

Records allow us to group a matrix's dimensions and its values. The approach here is similar to the one in {section:functions}: matrices are stored as mappings between indices and values, which allows for constant-time access to its elements.
