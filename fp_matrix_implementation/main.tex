...
Note that since the output of the function \lstinline{map2} is wrapped in the monadic result type \lstinline{result}, we must use \lstinline{Result.map} to apply \lstinline|sum|. Simlarly, we use standard monadic operations for the \lstinline|result| monad such as \lstinline|bind| or \lstinline|lift|.
...
formalisation in~\cite{de_maria_use_2021} considers properties of the form ``any neural network $f$ that satisfies a property $Q_1$, also satisfies a property
...
structure almost automatically (with the help of a handful of auxiliary lemmas). And the proofs easily go through for both quantised and real-valued neural networks. Note that for this proof, the implementation of weight matrices as lists of lists is implicit -- we redefine matrix manipulation functions that are less general but more convenient for proofs by induction.

\knote{Remi, please check it is true? Anything else needs to be said here?}
\rnote{It is true; I added a sentence clarifying that we do not use the exact same matrix implementation}

\section{Matrices as Functions} \label{sec:functions}

%Similarly to robustness properties, this verification property could be handled by general-purpose SMT solvers. However, as~\cite{KaBaDiJuKo17Reluplex} points out, they do not scale. Indeed, when we use our quantised library on quantised ACAS Xu neural networks, \lstinline{blast} does not terminate. 
%This is why, the algorithm \lstinline{reluplex} was introduced in \cite{KaBaDiJuKo17Reluplex} as an additional heuristic to SMT solver algorithms; \lstinline{reluplex} has since given rise to a domain specific solver Marabou~\cite{KatzHIJLLSTWZDK19}.

One weakness of the list-of-lists matrix implementation is the use of recursion with error checking in order to access elements. This creates case-splits whose number increases exponentially with the size of matrices. One way of eliminating this recursion and of providing constant-time access to matrix elements is to define matrices as functions from indices to values:
...
Incremental congruence closure and simplex data structures are shared across DAG nodes, and symbolic execution results are memoised. Moreover, forward-chaining rules (such as those characterizing `relu`) are only applied on demand.
...
A third implementation of matrices is by using OCaml records, a data structure that allows groups together values of different types. Records are available in Imandra, the only difference with the OCaml implementation is that it does not support function-types for values.
Records allow us to group a matrix's dimensions and its values. The approach here is similar to the one in {section:functions}: matrices are stored as mappings between indices and values. 
\begin{equation}
	m : \mathbb{N} \times \mathbb{N} \rightarrow \mathbb{R}
\end{equation}

However, instead of having the mapping be implemented as a function, here we implement it as a Map, i.e. an unorganised collection of of (key;value) pairs where each key is unique. 
...
As we want to have a total function in order to avoid having to check for dimensions, we set a default value at 0.

Similarly to the previous implementations, we define a number of useful matrix operations which will allow us to tersely define more complex functions. In order to iterate over matrix values, we use the dimensions and decrease in order to allow Imandra to prove termination. For instance, the 
