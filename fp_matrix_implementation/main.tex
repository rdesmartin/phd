...
And for that section, I really can't write much on the implementation detail as I'm still trying to understand parts of it. So if Grant finds the time to write it, good, otherwise can we sweep it under the rug in an honest way? i.e. we give a high-level explaination (we need a floor function to go from reals to integers in order to prove termination) but say we won't go into detail of this proof here?
...
\knote{Why 0, precisely? to have a matrix of zeros? please explain the motivation for this example.
  This could be a good moment to emphasise that indices are integers and values are reals, so can we add a comment, and may be have some meaningful real there?}
